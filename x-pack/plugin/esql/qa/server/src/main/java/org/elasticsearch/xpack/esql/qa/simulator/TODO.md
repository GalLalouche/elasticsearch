...
- [x] **SORT tie-breaking mismatch**: when sort keys are equal, simulator and ES break ties differently, leading to different rows being selected after LIMIT. Seeds: `-1487800040806453354`, `-3578431799500002710`. Fix: made result comparison order-insensitive — `extractSortedRows` builds row tuples from columnar data, sorts them lexicographically, then compares. This is correct because row order is nondeterministic when sort keys are tied or when there is no SORT.
- [ ] **Integer overflow**: simulator uses `long` arithmetic but ES uses `integer` which returns `null` on overflow. Example: `s1 * s1` where `s1=49000` → simulator gets `2401000000` (fits in long), ES returns `null` (overflows 32-bit int). Seed: `1934485686493129541` (with INLINE STATS/SORT bugs fixed, this becomes the first failure).
