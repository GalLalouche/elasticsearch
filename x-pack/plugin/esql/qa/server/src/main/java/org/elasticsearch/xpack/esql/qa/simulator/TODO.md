...
- [ ] **Deduplicate forced shrinker vs property-test shrinker**: `ForcedShrinkerIT` and `SimulatorPropertyIT.TestCaseGenerator.doShrink` each have their own copies of `flattenStages`, `rebuildFromStages`, expression shrinking, numeric-literal shrinking, etc. They drift (e.g., missing strategies on one side, bug fixes only applied to the other). Extract the strategies into a single `PlanShrinker` API that returns candidate `TestCase`s; have `doShrink` pass them through to junit-quickcheck and `ForcedShrinkerIT` consume them greedily in a loop.
...
- [ ] **Simulator diverges from ES on `0 * <overflow>`**: ES constant-folds `0 * X → 0` and never evaluates `X`, while the simulator evaluates left-to-right and propagates a null from overflow in `X`. Repro: `FROM sim_disdcc | STATS s1 = MIN(-(0 / 1) * (356134727 * b)) BY b` with `b=-10` → simulator=`[[-10, null]]`, ES=`[[-10, 0]]` (because `356134727 * -10 = -3561347270` overflows int). Decide whether the simulator should also short-circuit, or whether ES should propagate the null. Found via seed `2324831338214026207`.
